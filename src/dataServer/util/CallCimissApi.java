...
public class CallCimissApi {
	 ...
	 * 
	 ...
	 /****
	  * 根据检测时间，监测站代号、天气要素获取天气信息
	  * ***/
	 public static JSONObject getStationWeatherInfo(String time, String stationCode, String element) {
			// TODO Auto-generated method stub
		 String params="&interfaceId=getSurfEleByTimeAndStaID" /** 按时间、地区统计地面数据要素 */        		         
		         + "&elements=Station_ID_C,Station_Name,"+element/**站点ID、站点名称、经度、纬度、气象要素*/
		         + "&times="+time //检索时间
		         + "&staIds="+stationCode//检索地区
				 + "&dataCode=SURF_CHN_MUL_HOR";/**表示地面逐小时数据*/
		 JSONObject jsonData = getRstJson(params);
		 try {
				if(jsonData.getString("returnCode").equals("0")){
