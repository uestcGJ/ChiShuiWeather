...
import dataServer.voice.ucpass.rest.client.JsonReqClient;
...
public class VoiceMsgService {
	 ...
	 * 发送语音通知
	 * @param phone 输入被通知人员的手机号
	 * @param content 要发送短信的内容
	 * @param playTimes 语音播放的次数,可选1,2,3
	 * @return boolean status表示发送是否成功true表示成功
	 * **/
	public static boolean sendVoiceMessage(String phone,String content,int playTimes){
		...
		*/
		...
		try {
			JSONObject result = new JSONObject(jsonReqClient.voiceMessage(accountSid, authToken, appId, phone,type, templateId,msg.toString(),String.valueOf(playTimes)));
			if(((JSONObject) result.get("resp")).get("respCode").equals("000000")){
				status=true;
			}
		} catch (JSONException e) {
	 ...
	 * @return 格式为boolean  status表示发送是否成功true表示成功，false表示失败  
	 */
	public static boolean sendSms(String to, String param){	
		String templateId = "1";
		boolean status=false;
		try {
			JSONObject result = new JSONObject(jsonReqClient.templateSMS(accountSid, authToken,
